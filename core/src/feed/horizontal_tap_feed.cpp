...
int HorizontalTapFeed::getCurrentIndex() {
...
void HorizontalTapFeed::update(double dt) {
  ...
  gesture.withSingleTouch([&](const Touch &touch) {
    ...
    if (touch.released) {
      ...
      if (touchDuration < 0.2 && !touch.movedNear) {
        if (touch.screenPos.x > CARD_WIDTH / 2.0) {
        ...
        if (auto nextCardId = decks[idx].player->getScene().getNextCardId(); nextCardId) {
          API::loadCard(nextCardId->c_str(), true, [=](APIResponse &response) {
            if (response.success && idx == getCurrentIndex()) {
              ...
              decks[idx].player->readScene(reader);
            }
          });
...
void HorizontalTapFeed::renderCardAtPosition(int idx, float position, bool isActive) {
  if (idx < 0) {
    return;
  }
...
void HorizontalTapFeed::makeShader() {
...
void HorizontalTapFeed::draw() {
  ...
  playButton->draw(&lv.graphics,
      love::Matrix4(CARD_WIDTH * 0.7 - width * 0.5, CARD_HEIGHT + 70 - height * 0.5, 0, width,
          height, 0, 0, 0, 0));

  playButton->draw(&lv.graphics,
      love::Matrix4(CARD_WIDTH * 0.3 + width * 0.5, CARD_HEIGHT + 70 - height * 0.5, 0, -width,
          height, 0, 0, 0, 0));
}

void HorizontalTapFeed::fetchInitialDecks() {
  fetchingDecks = true;
  API::graphql("{\n  infiniteFeed {\n    sessionId\n    decks {\n      deckId\n      variables\n   "
               "   initialCard {\n        sceneDataUrl\n      }\n    }\n  }\n}",
...
void HorizontalTapFeed::fetchMoreDecks() {
  if (fetchingDecks) {
    return;
  }
...
void HorizontalTapFeed::loadDeckAtIndex(int i) {
...
void HorizontalTapFeed::unloadDeckAtIndex(int i) {
  if (i >= (int)decks.size() || i < 0) {
    return;
  }
...
love::graphics::Canvas *HorizontalTapFeed::newCanvas(int width, int height) {
...
void HorizontalTapFeed::renderToCanvas(
