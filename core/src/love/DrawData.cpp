...
void DrawData::addCircleSubpathData(PathData *pathData, float centerX, float centerY, float radius,
...
void DrawData::addFrame(OneIndexFrame frameIndex) {
  if (getNumLayers()) {
    ...
    for (auto &layer : layers) {
      ...
      if (zeroFrameIndex >= (int)layer->frames.size()) {
        ...
        layer->frames.push_back(std::move(newFrame));
        for (int ii = zeroFrameIndex; ii < (int)layer->frames.size() - 1; ii++) {
...
void DrawData::copyCell(std::shared_ptr<DrawDataFrame> sourceFrame, DrawDataLayerId destLayerId,
  ...
  if (destLayer && destFrameIndex.toZeroIndex() < (int)destLayer->frames.size()) {
...
void DrawData::copyCell(DrawDataLayerId sourceLayerId, OneIndexFrame sourceFrameIndex,
  ...
  if (sourceLayer && destLayer && sourceFrameIndex.toZeroIndex() < (int)sourceLayer->frames.size()
...
void DrawData::renderForTool(DrawDataLayerId layerId, OneIndexFrame frameIndex,
    float tempTranslateX, float tempTranslateY, std::shared_ptr<ToveGraphicsHolder> tempGraphics) {
