...
local function parseResources(code)
    ...
    for i = 1, #t do
        ...
        if
            a[1] == "identifier" and a[2] == "require" and b and
                (b[1] == "whitespace" or (b[1] == "operator" and b[2] == "(")) and
                c and
                c[1] == "string"
         then
            -- `require '...'` or `require('...'`
        ...
        elseif
            a[1] == "identifier" and resourceFuncs[a[2]] and b and b[1] == "operator" and b[2] == "(" and c and
                c[1] == "string" and
                d and
                (d[1] == "operator" and (d[2] == ")" or d[2] == ","))
         then
...
local LATEST_SHARE_LUA_URL =
    "https://raw.githubusercontent.com/castle-games/share.lua/526e86aeaa49da90539a173ad665325cebc27f73/cs.lua"
...
local function explicitRequire(path, opts)
    ...
    elseif network.exists(absolute .. "/init.lua", true) then
        ...
        error(
            "no working `url` found for '" ..
                origPath .. "' -- please check the `require` " .. "`path` or the network connection"
        )
    end
    ...
    if globalPackageCache[url] then
        return globalPackageCache[url]
    end
            ...
            if not parsePrefetchVisited[childEnv][resource.path] then
                ...
                network.async(
                    function()
                        pcall(
                            function()
                                -- Allow failure as we want the error to be hit on synchronous use
                                if resource.type == "lua" then
                                    childEnv.require(resource.path, {noEval = true})
                                elseif resource.type == "asset" then
                                    network.fetch(
                                        network.isAbsolute(resource.path) and resource.path or
    ...
    -- Save to cache
    if path == "vendor/tove" then
        print("SAVING VENDOR/TOVE")
