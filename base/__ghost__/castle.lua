...
if not CASTLE_SERVER then -- We're in the JS client, use the JS client's API calls
    ...
    function castle.storage.getGlobal(key)
        ...
        local err = result and type(result.errors) == "table" and result.errors[1] and result.errors[1].message
        if err then
            error("`castle.storage.getGlobal`: " .. err)
        end

        local json =
            result and type(result.data) == "table" and type(result.data.gameGlobalStorage) == "table" and
    ...
    function castle.storage.setGlobal(key, value)
        ...
        local success = result and type(result.data) == "table" and result.data.setGameGlobalStorage == true
        if not success then
...
function castle.post.get(options)
    return castle.post._decodePost(
        bridge.js.postGet {
