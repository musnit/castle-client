...
bool DrawDataFrame::floodFill(float x, float y) {
  ...
  auto pixelCount = getFillImageDataSizedToPathBounds()->floodFill(floor((x * parent()->fillPixelsPerUnit) - fillImageBounds.minX), floor((y * parent()->fillPixelsPerUnit) - fillImageBounds.minY), pathsImageData, p);
  compressFillCanvas();
...
void DrawDataFrame::renderToCanvas(graphics::Canvas * canvas, const std::function<void()> & lambda) {
...
void DrawDataFrame::updatePathsCanvas() {
  ...
  if (pathsCanvas == NULL || pathsCanvas->getWidth() != width || pathsCanvas->getHeight() != height) {
	...
	renderToCanvas(pathsCanvas, [bounds, this]() {
		graphics::Graphics *graphicsModule = Module::getInstance<graphics::Graphics>(Module::M_GRAPHICS);
