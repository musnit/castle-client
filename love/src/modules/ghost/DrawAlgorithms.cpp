    ...
    if (s1.type == line && s2.type == line) {
      ...
      if (d > -0.0001 && d < 0.0001) {
        ...
        if (areAnglesEqual(s1.startAngle, s2.endAngle)
          ...
          tempResults.push_back(Point(s1.center.x + (cos(s1.startAngle) * s1.radius),
          ...
          tempResults.push_back(Point(s1.center.x + (cos(s1.endAngle) * s1.radius),
        ...
        auto p2 = Point(s1.center.x + ((a * (s2.center.x - s1.center.x)) / d),
            s1.center.y + ((a * (s2.center.y - s1.center.y)) / d));
        if (h > -0.0001 && h < 0.0001) {
          tempResults.push_back(p2);
        } else {
          tempResults.push_back(Point(p2.x + ((h * (s2.center.y - s1.center.y)) / d),
      ...
      for (size_t i = 0; i < tempResults.size(); i++) {
        ...
        for (int j = -1; j < 1; j++) {
          auto add = (2 * M_PI) * j;
          if ((angle1 + add) >= (s1.startAngle - 0.001)
              && (angle1 + add) <= (s1.endAngle + 0.001)) {
            angle1Passed = true;
          }
          if ((angle2 + add) >= (s2.startAngle - 0.001)
              && (angle2 + add) <= (s2.endAngle + 0.001)) {
            angle2Passed = true;
          }
        }
