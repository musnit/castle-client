...
void DrawData::makeSubpathsFromSubpathData(PathData *pathData) {
  for (size_t i = 0; i < pathData->subpathDataList.size(); i++) {
	...
	if (subpathData.type == line) {
		...
		SubpathArc(subpath, subpathData.center.x, subpathData.center.y, subpathData.radius, (subpathData.startAngle * 180) / M_PI, (subpathData.endAngle * 180) / M_PI, false);
	}
  }
}
...
void DrawData::addSubpathDataForPoints(PathData *pathData, Point p1, Point p2) {
  ...
  if (bendPointOptional) {
	...
	auto distFromMidpointToBendPoint = sqrt(pow(midpointP1P2.x - bendPoint.x, 2) + pow(midpointP1P2.y - bendPoint.y, 2));
	...
	auto rayIntersectionResult = DrawAlgorithms::rayRayIntersection(p1Midpoint.x, p1Midpoint.y, p1Midpoint.x + p1NormalVector.x, p1Midpoint.y + p1NormalVector.y, p2Midpoint.x, p2Midpoint.y, p2Midpoint.x + p2NormalVector.x, p2Midpoint.y + p2NormalVector.y);
	...
	if (radius == 0) {
	  ...
	  addCircleSubpathData(pathData, circleCenter.x, circleCenter.y, radius, startAngle, startAngle + (M_PI / 2));
	  addCircleSubpathData(pathData, circleCenter.x, circleCenter.y, radius, startAngle + (M_PI / 2), startAngle + M_PI);
	} else {
	  Point circleCenter;
	  float startAngle;
	  if (p1.y > p2.y) {
...
void DrawData::runAnimation(AnimationState animationState, AnimationComponentProperties componentProperties, float dt, std::function<void(std::string)> fireTrigger, std::function<void()> fireChangedFrame) {
